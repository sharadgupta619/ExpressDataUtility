...
class ConfigurationFile(@JsonProperty("metrics") _metrics: Seq[String],
  ...
  //val inputs: Seq[Input] = Option(_inputs).getOrElse(Map()).map { case (k, v) => Input(k, v) }.toSeq
  val inputs: Seq[Input] = Option(_inputs) match {
    case None => Seq()
    case Some(_inputs) => _inputs.map { case (k, v) => Input(k, v) }.toSeq
  }
  ...
  val appName: String = Option(_appName).getOrElse("ExpressDataUtility")
